...
class NotificationService {
  ...
  static Future<ApiReturnValue<AdvisoryModel>> rateApp(String bookid, int rate,
      {http.Client? client}) async {
    String url = baseURL + "tadvisory/rateapp/$bookid/$rate";
    client ??= http.Client();
    ...
    log("responnyaa rateApp ${response!.body}");
    if (response.statusCode != 200) {
      return ApiReturnValue(message: 'Please try again');
    }
    ...
    AdvisoryModel model = AdvisoryModel.fromJson(data['data']);

    return ApiReturnValue(value: model, message: "");
  }
}
